...
- **Ready Player Me 3D Avatars:** Integrated Ready Player Me avatars in AR Holo Mode. Users set avatar GLB URLs in profile settings. When entering AR during calls, remote participants' avatars load automatically using GLTFLoader and render with proper lighting (ambient + directional lights). Avatars stand independently in the room (positioned 0.5m to the right of video plane), not parented to video feeds. Independent gesture controls allow dragging and scaling avatars separately from video planes via touch raycasting. Avatars scale uniformly while preserving original textures and materials. Comprehensive texture disposal prevents GPU memory leaks when exiting AR mode. LiveKit identity uses actual Supabase user UUID for avatar loading.
