import {
  Room, RoomEvent, createLocalTracks, setLogLevel
...
/* show Holo button when remote present on either peer */
...
function previewLocal(tracks){
  ...
  if(vt && typeof vt.attach==='function'){ vt.attach(v); } else { const stream=new MediaStream(tracks.map(t=>t.mediaStreamTrack)); v.srcObject=stream; v.play().catch(()=>{}); }
...
function attachRemoteTrack(track, pub, participant){
  if(pub.kind === 'video'){
    ...
    // mirror to hidden holo texture source
    const hv=$('remoteHoloVideo'); hv.srcObject = rv.srcObject;
...
async function join(){
  ...
  await fetchConfig(); const token=await getToken(roomName, userName);
  setLogLevel('warn');
  ...
  room.on(RoomEvent.ParticipantConnected,  (p)=>{ showToast(`${p.identity} joined`); recalcHoloVisibility(); });
  room.on(RoomEvent.ParticipantDisconnected,(p)=>{ showToast(`${p.identity} left`);   recalcHoloVisibility(); });

  room.on(RoomEvent.TrackSubscribed,   (track,pub,p)=> attachRemoteTrack(track,pub,p));
  room.on(RoomEvent.TrackUnsubscribed, ()=> { const rv=$('remoteVideo'); if(rv) rv.srcObject=null; recalcHoloVisibility(); });

  const localTracks=await createLocalTracks({ audio:true, video:{ facingMode:'user', width:960, frameRate:24 } });
  state.localTracks=localTracks; previewLocal(localTracks);

  await room.connect(state.livekitUrl, await token);
  for(const t of localTracks) await room.localParticipant.publishTrack(t);

  setUIState({ joined:true }); showToast(`joined: ${roomName}`);
  recalcHoloVisibility(); // ensure both peers see Holo button appropriately
}
async function leave(){
  try{ if(state.room) await state.room.disconnect(); for(const t of state.localTracks){ try{ t.stop(); }catch{} } }
    ...
    const lv=$('localVideo'); const rv=$('remoteVideo'); const hv=$('remoteHoloVideo');
...
const overlay=$('confirmOverlay'); const confirmCancel=$('confirmCancel'); const confirmLeave=$('confirmLeave');
...
/* ==========================================================
   Holo Mode (WebXR manual session, no ARButton)
   ========================================================== */
...
async function startHoloMode(){
  ...
  try{
    // 1) request XR session on user gesture
    ...
    // 4) attach session to renderer, spaces & hit test
...
function cleanupXR(){
  ...
  xrSession=refSpace=hitTestSource=null;
}
